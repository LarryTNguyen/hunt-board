...
export const api = {
  ...
  retryFailedSources: (id) => request(`/admin/scrape-runs/${id}/retry-failed`, { method: 'POST', body: {} }),
  cancelRun: (id) => request(`/admin/scrape-runs/${id}/cancel`, { method: 'POST', body: {} }),
  recoverStaleRuns: () => request('/admin/ingestion/recover-stale', { method: 'POST', body: {} }),
  quarantines: (status = 'pending') => request(`/admin/quarantines?status=${encodeURIComponent(status)}`),
  approveQuarantine: (id, note = null) => request(`/admin/quarantines/${id}/approve`, { method: 'POST', body: { note } }),
  rejectQuarantine: (id, note = null) => request(`/admin/quarantines/${id}/reject`, { method: 'POST', body: { note } }),
  correlationLookup: (identifier) => request(`/admin/operations/correlation?identifier=${encodeURIComponent(identifier)}`),
